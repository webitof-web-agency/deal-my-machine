...
import { extractIdFromSlug, generateDealerSlugPath } from '@/lib/seoUtils';
...
export async function generateMetadata({
  ...
  const dealerId =
    (await resolvePublicDealerId(rawDealerId)) || extractIdFromSlug(rawDealerId);
  const dealer = await getDealerDetail(dealerId);
...
export default async function DealerDetailPage({ params }: DealerDetailPageProps) {
  ...
  const listings = await getDealerListings(dealerLookupId);
